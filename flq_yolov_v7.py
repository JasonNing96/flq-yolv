...
def generate_out_dir(model_path: str, bits: int, local_epochs: int, base_dir: str = "./results/runs_flq_v7") -> str:
    """根据参数自动生成详细的输出文件夹名称"""
    ...
    # 组合文件夹名称：runs_flq_v7_yolov8s_32bit_1epoch
    detailed_name = f"runs_flq_v7_{model_name}_{bits}bit_{local_epochs}epoch"
    
    # 如果 base_dir 是默认值，使用详细名称；否则使用用户指定的
    if base_dir == "./results/runs_flq_v7":
